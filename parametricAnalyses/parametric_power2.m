...
listAmpPower = 0:20:120

nbIteration = length(listAmpPower);
for iteration=1:nbIteration
    disp(['----------------------------------ITERATION ' num2str(iteration) '/'  num2str(nbIteration)  ' ---------------------------------']);
    PAR.powerGen.amplitude.value = listAmpPower(iteration);
    ...
    %% Build model
    model.modelname = ['model_' PAR.nameTest '_' num2str(N) 'step_' num2str(tf) 's_memEmpty_amp_' num2str(PAR.powerGen.amplitude.value) 'W' ];
