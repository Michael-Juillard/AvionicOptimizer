...
listPeriodPower = 10:10:70;

nbIteration = length(listPeriodPower);
for iteration=1:nbIteration
    disp(['----------------------------------ITERATION ' num2str(iteration) '/'  num2str(nbIteration)  ' ---------------------------------']);
    PAR.powerGen.period.value = listPeriodPower(iteration);
    ...
    %% Build model
    model.modelname = ['model_' PAR.nameTest '_' num2str(N) 'step_' num2str(tf) 's_memEmpty_period_' num2str(PAR.powerGen.period.value) 'steps' ];
