...
%% parameters
PAR = loadParamXML("param/param_mem_empty_v3.xml");

listWeightChange = 0.4:0.2:1.6;

nbIteration = length(listWeightChange);
for iteration=1:nbIteration
    disp(['----------------------------------ITERATION ' num2str(iteration) '/'  num2str(nbIteration)  ' ---------------------------------']);
    PAR.objFuncWeight.value{1}(1) = listWeightChange(iteration);
    PAR.objFuncWeight.value{1}(2) = 2- listWeightChange(iteration);
    ...
    %% Build model
    model.modelname = ['model_' PAR.nameTest '_' num2str(N) 'step_' num2str(tf) 's_memEmpty_weight_' num2str(PAR.objFuncWeight.value{1}(1)*10) 'div10' ];
