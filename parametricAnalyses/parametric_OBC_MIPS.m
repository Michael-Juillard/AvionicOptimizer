...
OBC_mode2_MIPS = PAR.arch.OBC.procRess.value(2);
OBC_mode3_MIPS = PAR.arch.OBC.procRess.value(3);

list_MIPS_perc = 0.4:0.2:1.6;

nbIteration = length(list_MIPS_perc);
for iteration=1:nbIteration
    disp(['----------------------------------ITERATION ' num2str(iteration) '/'  num2str(nbIteration)  ' ---------------------------------']);
    PAR.arch.OBC.procRess.value(2) = list_MIPS_perc(iteration)*OBC_mode2_MIPS;
    PAR.arch.OBC.procRess.value(3) = list_MIPS_perc(iteration)*OBC_mode3_MIPS;
    ...
    %% Build model
    model.modelname = ['model_' PAR.nameTest '_' num2str(N) 'step_' num2str(tf) 's_memEmpty_OBC_MIPS_' num2str(list_MIPS_perc(iteration)*100) 'perc' ];
