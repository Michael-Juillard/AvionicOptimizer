...
listMinPower = 220:20:300;

nbIteration = length(listMinPower);
for iteration=1:nbIteration
    disp(['----------------------------------ITERATION ' num2str(iteration) '/'  num2str(nbIteration)  ' ---------------------------------']);
    PAR.powerGen.offset.value = listMinPower(iteration);
    ...
    %% Build model
    model.modelname = ['model_' PAR.nameTest '_' num2str(N) 'step_' num2str(tf) 's_memEmpty_offset_' num2str(PAR.powerGen.offset.value) 'W' ];
